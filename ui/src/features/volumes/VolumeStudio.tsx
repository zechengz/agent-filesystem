...
import { BrowseTab } from "../../routes/workspace-studio/-browse-tab";
import { CheckpointsTab } from "../../routes/workspace-studio/-checkpoints-tab";
import { ChangesTab } from "../../routes/workspace-studio/-changes-tab";
import { SettingsTab } from "../../routes/workspace-studio/-settings-tab";
import { SearchTab } from "../../routes/workspace-studio/-search-tab";

export const volumeStudioSearchSchema = z.object({
  tab: studioTabSchema.optional(),
  welcome: z.boolean().optional(),
  databaseId: z.string().optional(),
});

export type VolumeStudioBreadcrumbItem = {
  label: string;
  onClick: () => void;
};

export type VolumeStudioNavigateOptions = {
  tab: StudioTab;
  replace?: boolean;
};

type VolumeStudioProps = {
  volumeId: string;
  databaseId?: string | null;
  activeTab?: StudioTab;
  welcome?: boolean;
  breadcrumbItems: VolumeStudioBreadcrumbItem[];
  onNavigateToSelf: (options: VolumeStudioNavigateOptions) => void;
  onDeleted: () => void;
};

export function VolumeStudio({
  ...
  const showWelcomeInterstitial =
    workspace != null && welcome === true && !userRequestedBanner;
  ...
  function setStudioTab(nextTab: StudioTab) {
    onNavigateToSelf({ tab: nextTab, replace: true });
  }
  ...
  return (
    <PageStack>
          ...
          <WelcomeCard>
            ...
            <WelcomeActions>
              <Button
                ...
                onClick={() => {
                  onNavigateToSelf({ tab, replace: true });
                }}
              >
                I&apos;ll do this later
              </Button>
      ...
      <StudioNavRow>
        <BreadcrumbGroup aria-label="Breadcrumb">
          <BreadcrumbList>
...
const BreadcrumbGroup = styled.nav`
  display: flex;
  align-items: center;
`;
